	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(isLogin!=null) {
			if(op.equals("newWriting")) {
				...
				System.out.println(request.getParameter("writing_title"));
				writing.setWriting_content(request.getParameter("writing_content"));
				System.out.println(request.getParameter("writing_content"));
				writing.setWriting_writer(request.getParameter("writing_writer"));
				System.out.println(request.getParameter("writing_writer"));
				writing.setWriting_history(request.getParameter("writing_writer"));
				if(WritingDAO.createWriting(writing)) {
					pageLocation = "boardMain.jsp";
				} else {
					pageLocation = "error.jsp";
					request.setAttribute("errorMsg", "글 쓰기에 실패했습니다.");
				}
			}
		} else {
			try {
